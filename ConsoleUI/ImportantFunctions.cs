...
namespace ConsoleUI
{
	public static class ImportantFunctions
	{
		...
		public static void LoginScreen()
		{
			...
			Text debugTxt = new Text(debugMessage, new Vector2(0, 15));
			...
			debugBtn.OnClickEvent += (bsender, args) => { DebugButton(); };
			void DebugButton()
			{
				string ButtonOrder = "";

				for(int i=0;i<ConsoleItems.AllInputFieldItems.Count;i++)
				{
					ButtonOrder += $"{ConsoleItems.AllInputFieldItems[i].title} ";
				}

				for(int i=0;i<ConsoleItems.AllClickableItems.Count;i++)
				{
					if(!(ConsoleItems.AllClickableItems[i] is InputField))
					{
						ButtonOrder += $"{ConsoleItems.AllClickableItems[i].text} ";
					}
				}

				debugTxt.ChangeText(ButtonOrder);
			}
		}
